/*!
	This datum should be used for handling material contents of machines and whatever else is supposed to hold materials and make use of them.

	Variables:
		amount - raw amount of the material this container is holding, calculated by the defined value MATERIAL_AMOUNT=2000.
		max_amount - max raw amount of material this container can hold.
		sheet_type - type of the material sheet the container handles, used for output.
		parent - object that this container is being used by, used for output.
		MAX_STACK_SIZE - size of a stack of material sheets. Constant.
*/
...
/datum/component/material_container/proc/examine(datum/source, mob/user)
	SIGNAL_HANDLER

	if(show_on_examine)
		for(var/I in materials)
			var/singleton/material/M = I
			var/amt = materials[I]
			if(amt)
...
/datum/component/material_container/proc/AttackBy(datum/source, obj/item/I, mob/living/user)
	...
	if(!has_space(material_amount))
		...
		return
	if(I.contents.len && !istype(I, /obj/item/stack) && !istype(I, /obj/item/ammo_pile) && !istype(I, /obj/item/ammo_magazine))
...
/datum/component/material_container/proc/insert_item(obj/item/I, multiplier = 1, stack_amt)
	if(!I || !I.recyclable || !I.matter)
		to_chat(user, SPAN_WARNING("\The [I] cannot be recycled by \the [parent]."))
		return FALSE
...
/// For spawning material sheets at a specific location. Used by machines to output sheets.
	...
	for(var/x in mats) //Loop through all required materials
		...
		if(!istype(req_mat))
			if(ispath(req_mat)) //Is this an actual material, or is it a category?
				req_mat = GET_SINGLETON(req_mat) //Get the ref
...
/datum/component/material_container/proc/get_material_amount(singleton/material/mat)
	if(!istype(mat))
